...
npm run validate-course-md # syllabus-first Theme / Syllabus sources / _raw gaps
...
- **sync** = source of truth for `inbox/week.md` (dated, open work in the window; America/Denver days) and refreshes **Assignment catalog** + **Checkpoints** in `inbox/courses/*.md` for course-arc briefings. **Due column** in week.md and catalogs is **America/Denver wall-clock (MT)**, not raw UTC. Also fetches per-course **syllabus** (→ `inbox/courses/_raw/CODE-syllabus.txt`), and when the Canvas syllabus page is a stub, merges **supplement pages** (syllabus/grading/policies) and **syllabus PDF files** (plus local `_raw/CODE-syllabus.pdf`). Writes **instructors** and **policy page** links into course file headers and `### Policy pages (synced)`. When COEN 1500 is enrolled, also runs **sync-dinners**. After sync: run `jacob-syllabus-intake` when hash changes, then `validate-profiles` + `validate-course-md`.
