...
| 3 | Tauri ambient-dock model | **Keep and finish** Hidden-as-default and narrate-auto-peek. **Park:** leave peek/expanded as-built and mark the rest design-only. **Replace:** new shell — do not start without this signature. | [`docs/design/ambient-dock-ui.md`](../design/ambient-dock-ui.md) is partially built. Demo STOP / NarrateAfter / fake ledger / approve-demo were **removed 2026-09-12** (pre-ship honesty). Hidden-as-default and real narrate-auto-peek still unsigned. | unsigned |
...
|---|-------|-------|-----------|------|
| 1 | Git-history purge of the Jacob corpus | Explicit approval to rewrite and force-push `phase1-productname-pivot` | [`history-purge.md`](./history-purge.md). Local `pre-purge-backup` tag deleted 2026-09-12; `git show 37f38b3:dev/JACOB.md` still returns the profile. Do not merge this tip to a public default branch until this completes. | |
| 2 | Live Google OAuth smoke | Real Google Cloud OAuth client; do not commit secrets or `token.json` | [`oauth-smoke.md`](./oauth-smoke.md). Dry-run is tested; live consent is not. Checklist is **read/draft only** (create-event/send are hard-blocked). | |
...
## Agent progress (2026-09-12) — not signatures

Pre-ship audit implementation closed agent-doable gates: discussion + photo submit preview-only; demo dock chrome removed; waitlist/onboarding honesty; Privacy/Terms drafts in [`docs/legal/`](../legal/); CI runs on `phase1-productname-pivot`; local `pre-purge-backup` tag deleted. **Rows 1–4 above remain unsigned.** Human walk items 1–5 remain unchecked.
