    ...
    <section className="study" aria-labelledby="study-heading">
      <header className="study-header">
        ...
        <div className="study-controls">
          <fieldset className="segmented" aria-label="Session length" disabled={phase.kind === "attempt"}>
...
function ElapsedClock({ startedAt, plannedSeconds }: { startedAt: string; plannedSeconds: number }) {
  ...
  return (
    <p className="elapsed mono" aria-live="off">
      {fmtSeconds(elapsed)} of {fmtSeconds(plannedSeconds)} planned{elapsed > plannedSeconds ? " — take the time you need" : ""}
    </p>
  );
}
...
function AttemptPanel({
  ...
  return (
    ...
    >
      ...
      <ElapsedClock startedAt={attempt.started_at} plannedSeconds={planned} />
...
function FeedbackPanel({ result, item, announce, onNext }: { result: Assessed; item: PublicItem; announce: (t: string) => void; onNext: () => void }) {
  ...
  return (
    <div className="panel feedback">
      ...
      <h2 ref={heading} tabIndex={-1}>
        {disagreed ? "Saved as uncertain" : OUTCOME_LABEL[a.outcome]}
