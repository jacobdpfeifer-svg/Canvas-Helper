...
5. **Gate writes** — any MCP (or equivalent) tool that writes to / acts on the student account in a way **visible only to the student** (e.g. local prefs, self-only Canvas toggles, personal Gmail send, personal Google Calendar create/update) must use `ConfirmationGuard` via `canvas_mcp.core.connector_guards.get_connector_guard(connector_id)` (preview → fingerprint → issue → confirm → reserve), with no automatic/standing posture — a fresh per-instance human confirm every time. No “safe because mostly read-only” exemption on the first write-capable action. Tools whose Canvas effect would be visible to someone else (submit, comment, discussion post) must stay **preview-only / hard-blocked** — do not build a ConfirmationGuard execute path for them (canvas-focus pivot, 2026-09-13 addendum draws this line at Canvas vs personal-account, not at "visible to anyone" in general).
